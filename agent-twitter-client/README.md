...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("tohsaka_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("tohsaka_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("tohsaka_RIG", 20, None).await?;
...
Created by [tohsaka](https://x.com/tohsaka_RIG)
